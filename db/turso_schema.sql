...
-- Inline documentation: This table is the authoritative log for each leaderboard prize distribution attempt. All columns must match backend code expectations.
-- Do not change columns without updating backend service logic.
-- Columns 'distribution_type', 'total_prize_pool', 'logged_at', 'updated_at' are intentionally omitted for strict alignment.
-- Timestamps are stored as TEXT (ISO 8601) for cross-platform compatibility.
-- Example insert: INSERT INTO distribution_summary_log (id, period_identifier, board_type, status, total_prize_pool_claimed, started_at) VALUES (...)
-- See backend PrizeDistributionService for all usage.
...
-- Inline documentation: This table logs every individual payout for auditing and troubleshooting.
-- Columns 'tx_hash', 'status', 'error_message', 'logged_at' are omitted unless backend code is updated to use them.
